...
for($i=2;$i<=$row;$i++){
	...
	$jenjang 							=$data->val($i,3);	
	$prodi 							 	=$data->val($i,4);	
	$id_jenis_daftar 					=$data->val($i,5);
	$id_jalur_daftar 					=$data->val($i,6);
	$id_periode_masuk 					=$data->val($i,7);
	$tanggal_daftar						=$data->val($i,8);
	$sks_diakui							=$data->val($i,9);
	$nama_pt_asal						=$data->val($i,10);
	$nama_prodi_asal 					=$data->val($i,11);
	$id_pembiayaan					 	=$data->val($i,12);	 			
$sql="insert into updateriwayatpendidikanmahasiswa (nim,nama_mahasiswa,jenjang,prodi,id_jenis_daftar,id_jalur_daftar,id_periode_masuk,tanggal_daftar,sks_diakui,nama_pt_asal,nama_prodi_asal,id_pembiayaan,status) values('$nim','$nama_mahasiswa','$jenjang','$prodi','$id_jenis_daftar','$id_jalur_daftar','$id_periode_masuk','$tanggal_daftar','$sks_diakui','$nama_pt_asal','$nama_prodi_asal','$id_pembiayaan','0')";
$cn->query($sql);
echo $cn->error;
}
}
 ...
 ?>
				<span id="loadingg"></span><a class="btn btn-xs btn-danger" href="../piksi/mahasiswa/validasi_riwayat_pendidikan.php"><span class="glyphicon glyphicon-check"></span> 3. Validasi</button>
				<span id="loadingimportfeeder"></span><a href='piksi/../mahasiswa/import_mahasiswa_history_pendidikan.php' class="btn btn-xs btn-primary" id="import_feeder"><span class="glyphicon glyphicon-paperclip"></span> 4. Update History Pendidikan</a>
